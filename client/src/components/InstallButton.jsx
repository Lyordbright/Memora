...
export default function InstallButton() {
  ...
  return (
    <>
      ...
      >
        {/* Idle pulsing glow ring, starts once the entrance animation settles */}
        <motion.span
          className="absolute inset-0 rounded-full bg-brand-gradient"
          initial={{ opacity: 0 }}
          animate={{ opacity: [0, 0.5, 0], scale: [1, 1.35, 1.35] }}
          transition={{ delay: 2, duration: 2.2, repeat: Infinity, repeatDelay: 1.6, ease: 'easeOut' }}
        />
        <motion.button
          onClick={handleClick}
          whileHover={{ scale: 1.04 }}
          whileTap={{ scale: 0.97 }}
          className="relative flex items-center gap-2 bg-brand-gradient text-white font-semibold text-sm pl-4 pr-5 py-3 rounded-full shadow-glow"
        >
          <Download size={16} />
          Download App
        </motion.button>
